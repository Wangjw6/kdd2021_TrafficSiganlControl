...
n_ant = 859
n_actions = 8
observation_space = 24 #+ 4 + n_actions
hidden_dim = 400
...
# adj = np.load('./agent/adj.npy').reshape(1,n_ant,n_ant)
...
if __name__ == '__main__':
    ...
    while i_episode < epoch:
        ...
        now_phase = dict.fromkeys(list(agent_hash.keys()), 1)
        ...
        for key in agent_hash.keys():
            ...
            inet_type[n] = 1
            obs[agent_hash[key]] = np.array(observations_for_agent[key]['lane_vehicle_num']).reshape(-1, ).tolist()
                                   #+ np.array(observations_for_agent[key]['lane_speed']).reshape(-1, ).tolist()
        ...
        while steps < mx_step:
            ...
            penalty = [0. for _ in range(n_ant)]
            delay_sum = []
            for _ in range(2):
            ...
            for key, val in next_observations.items():
                ...
                observations_for_agent[observations_agent_id][observations_feature] = val[1:]
            for key in agent_hash.keys():
                next_obs[agent_hash[key]] = np.array(observations_for_agent[key]['lane_vehicle_num']).reshape(-1, ).tolist()
                               #+inet_type.tolist() +agent_last_action.tolist()
            ...
            # c = 0
            # for key in agent_hash.keys():
            #     reward[agent_hash[key]] = np.exp(-penalty[agent_hash[agent]])
            #     # if reward[agent_hash[key]]+penalty[agent_hash[key]]==0:
            #     #     reward[agent_hash[key]] = 1.
            #     # else:
                #     reward[agent_hash[key]] = reward[agent_hash[key]]/(reward[agent_hash[key]]+penalty[agent_hash[key]])
            # print(c,c/n_ant)
        ...
        for e in range(n_epoch):
            ...
            # Double Q setting
            target_action_indices = model(torch.Tensor(Next_O) , torch.Tensor(Next_Matrix) ).max(dim=2)[1]
            target_q_values = model_tar(torch.Tensor(Next_O) , torch.Tensor(Next_Matrix) ).gather(2, target_action_indices.view(-1,n_ant,1)).detach()
            target_q_values = np.array(target_q_values.cpu().data)
            ...
            loss = (q_values - torch.Tensor(expected_q).detach()).pow(2).mean()  # - 0.0001*entropy.sum()
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
