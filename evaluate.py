...
if __name__ == "__main__":
    ...
    try:
        ...
        json.dump(result,open(scores_dir / "scores.json",'w'),indent=2)
        raise AssertionError()
    agent_spec['test'].load_model(args.m)
    ...
    try:
        ...
        scores_df['episode'] = [args.m]
        scores_df['scores'] = [scores[1]]
        scores_df['served'] = [scores[0]]
        scores_df.to_csv('scores.csv', mode='a', header=False)
    except Exception as e:
        msg = format_exception(e)
        result['error_msg'] = msg
        json.dump(result,open(scores_dir / "scores.json",'w'),indent=2)
