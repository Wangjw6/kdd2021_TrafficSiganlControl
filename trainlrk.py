...
if __name__ == '__main__':
    ...
    while i_episode < epoch:
        ...
        for e in range(n_epoch):
            ...
            target_q_values = model_tar(torch.Tensor(Next_O), torch.Tensor(Next_Matrix))
            target_q_values_lowrank = []
            for j in range(batch_size):
                q = target_q_values[j,:,:]
                q = torch.svd_lowrank(q, q=2)
                q = torch.mm(torch.mm(q[0], torch.diag(q[1])), q[2].t())
                target_q_values_lowrank.append(q)
            target_q_values_lowrank = torch.stack(target_q_values_lowrank, dim=0)
            target_q_values = target_q_values_lowrank.max(dim=2)[0]
